...
router.post('/checkout/buyer-unlock', authMiddleware, async (req, res) => {
  try {
    if (req.user.role !== 'buyer') {
      return res.status(403).json({ message: 'Only buyers can start this checkout.' });
    }
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${front}/browse?unlock_payment=success&session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${front}/browse?unlock_payment=cancelled`,
      metadata: {
        type: 'buyer_platform_unlock',
    ...
    console.error('buyer-unlock checkout error:', err);
    return res.status(500).json({ message: 'Error creating checkout session' });
  }
});

// Create checkout session (per-property unlock)
