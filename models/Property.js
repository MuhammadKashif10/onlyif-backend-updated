...
const propertySchema = new mongoose.Schema({
  ...
  address: {
    ...
    zipCode: {
      ...
      trim: true,
      // US: 12345 or 12345-6789 | AU: 4 digits (e.g. 3000)
      match: [/^\d{4}$|^\d{5}(-\d{4})?$/, 'Please provide a valid postcode (AU: 4 digits, US: 5 or 9 digits)']
    },
    country: {
      type: String,
      default: 'AU',
  ...
  status: {
    type: String,
    enum: ['draft', 'pending', 'review', 'active', 'buyer-interest', 'negotiation', 'under-offer', 'sold', 'withdrawn', 'rejected'],
    default: 'pending'
  },
...
propertySchema.pre('save', function(next) {
  // Auto-generate title if empty: "4 Bedroom Home in Chelsea"
  if (!this.title || !this.title.trim()) {
    const typeLabel = (this.propertyType || 'home').replace(/-/g, ' ');
    const city = this.address?.city || 'Property';
    this.title = `${this.beds || 0} Bedroom ${typeLabel.charAt(0).toUpperCase() + typeLabel.slice(1)} in ${city}`;
  }
  ...
  if (this.isModified('title') || this.isModified('address')) {
    const title = this.title || 'property';
    const slugBase = `${title}-${this.address?.city || ''}-${this.address?.state || ''}`
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-+|-+$/g, '') || 'listing';
