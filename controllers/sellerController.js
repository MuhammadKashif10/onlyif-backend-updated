...
const getSellerListings = async (req, res) => {
  try {
    ...
    // Get buyer activity metrics for all properties
    const propertyIds = propertiesRaw.map((p) => p._id);
    const [purchaseUnlocks, txUnlocks, inspectionCounts, savedCounts] = await Promise.all([
      Purchase.aggregate([{ $match: { property: { $in: propertyIds }, status: 'paid' } }, { $group: { _id: '$property', count: { $sum: 1 } } }]),
      Transaction.aggregate([{ $match: { property: { $in: propertyIds }, status: 'succeeded', 'items.addonType': 'property_unlock' } }, { $group: { _id: '$property', count: { $sum: 1 } } }]),
      Inspection.aggregate([{ $match: { property: { $in: propertyIds } } }, { $group: { _id: '$property', count: { $sum: 1 } } }]),
      User.aggregate([{ $match: { favorites: { $in: propertyIds } } }, { $unwind: '$favorites' }, { $match: { favorites: { $in: propertyIds } } }, { $group: { _id: '$favorites', count: { $sum: 1 } } }])
    ]);
    const mergeCounts = (a, b) => {
      const m = {};
      [...a, ...b].forEach((x) => { m[x._id.toString()] = (m[x._id.toString()] || 0) + x.count; });
      return m;
    };
    const unlockMap = mergeCounts(purchaseUnlocks, txUnlocks);
    const inspectionMap = Object.fromEntries(inspectionCounts.map((x) => [x._id.toString(), x.count]));
    const savedMap = Object.fromEntries(savedCounts.map((x) => [x._id.toString(), x.count]));
    ...
    const properties = propertiesRaw.map((p) => {
      ...
      return {
        ...
        buyerActivity: {
          views: p.viewCount || 0,
          unlocks: unlockMap[pid] || 0,
          inspectionRequests: inspectionMap[pid] || 0,
          saved: savedMap[pid] || 0,
        },
      };
    });
