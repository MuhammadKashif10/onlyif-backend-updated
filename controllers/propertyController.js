...
// Helper: Get approximate address (city, state postcode) - hide street until unlock
const getApproximateAddress = (addr) => {
...
const shouldShowFullAddress = async (propertyId, userId, userRole, roles = []) => {
  ...
  const txUnlock = await Transaction.findOne({
    user: userId,
    property: propertyId,
    'items.addonType': 'property_unlock',
    status: 'succeeded'
  });
  if (txUnlock) return true;
...
const getPropertyById = async (req, res) => {
  try {
    ...
    const addressString = showFull
      ...
      : getApproximateAddress(propertyObj.address);
...
const getAllProperties = async (req, res) => {
  ...
  // List view always shows approximate address (city, state, postcode) - full address only after unlock
  const normalizedProperties = properties.map(property => {
    const propertyObj = property.toObject();
    
    const addressString = getApproximateAddress(propertyObj.address);
    ...
    // Contact phone hidden in list - only shown after unlock
...
const createPropertyWithFiles = async (req, res) => {
  ...
  try {
    ...
    // Comprehensive field validation (title optional - auto-generated if empty)
    const requiredFields = {
    ...
    try {
      ...
      // Validate postcode: AU (4 digits) or US (5 or 9 digits)
      const zipRegex = /^\d{4}$|^\d{5}(-\d{4})?$/;
      if (!zipRegex.test(zipCode.trim())) {
        invalidFields.push('Postcode must be AU format (4 digits, e.g. 3000) or US format (12345 or 12345-6789)');
      }
    ...
    const isAUPostcode = /^\d{4}$/.test(zip);
    const propertyData = {
      ...
      address: {
        ...
        state: state.trim().toUpperCase(),
        zipCode: zip,
        country: isAUPostcode ? 'AU' : 'US'
      },
      location: {
        type: 'Point',
        coordinates: isAUPostcode ? [145.0, -37.8] : defaultCoordinates
      },
      price: parseFloat(price),
      beds: parseInt(beds),
      baths: parseFloat(baths),
